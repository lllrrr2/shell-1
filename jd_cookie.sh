...
start_jd_cookie(){
        ...
        echo "   1) 使用--link模式启动(redis容器和jd_cookie容器在同一主机，服务器一般推荐使用该模式)"
        echo "   2) 以普通模式启动"
        echo "   0) 退出"
        echo -ne "\n你的选择："
        read param
...
check_update() {
  new_version=$(curl -s "https://ghproxy.com/https://raw.githubusercontent.com/yuanter/shell/main/jd_cookie/version")

...
echo "   1) 安装|升级jd_cookie(自带安装redis容器)"
...
case $input in
    ...
    1)	echo -e "${yellow}正在拉取安装jd_cookie脚本${plain}";
		...
		case $is_speed_one in
			1) 	echo "国内模式下载安装脚本中。。。"
				wget -O onekey_jd_cookie.sh  --no-check-certificate https://ghproxy.com/https://raw.githubusercontent.com/yuanter/shell/main/jd_cookie/onekey_jd_cookie.sh >/dev/null 2>&1
				chmod +x *sh
				bash onekey_jd_cookie.sh
			;;
			2) 	echo "国外模式下载安装脚本中。。。"
				wget -O onekey_jd_cookie.sh  --no-check-certificate https://raw.githubusercontent.com/yuanter/shell/main/jd_cookie/onekey_jd_cookie.sh >/dev/null 2>&1
				chmod +x *sh
